...
export const SENTENCE_TRANSLATE_OPTIONS_RULE =
  'sentence-translate 的 answers 必须按自然中文译文的阅读顺序排列，例如「昨日、映画を見た。」为 ["昨天","看了","电影"]。options 必须按词卡数量完整包含 answers：answers 中同一个中文词语每出现一次，options 中也必须至少出现一次。重复词语不得去重。例如 answers 为 ["我","让","妹妹","叫醒了","我"] 时，options 中必须提供两个 "我" 词卡。options 和 answers 中的每个词卡不得包含标点符号。';
